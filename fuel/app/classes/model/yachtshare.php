...
*/
	...
	{
		$query = DB::query("SELECT url FROM `images` WHERE belongs_to='yachtshare' AND belongs_to_id=".$this->id.' AND type="public_header"');
	...
	public function get_public_image_urls_except_header()
	{
		$query = DB::query("SELECT url FROM `images` WHERE belongs_to='yachtshare' AND belongs_to_id=".$this->id.' AND type="public_gallery"');
		$result = $query->as_assoc()->execute();
		return $result;
	}
